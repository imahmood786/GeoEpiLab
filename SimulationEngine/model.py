...
class InfectedModel(Model):
    ...
    geojson_agents = "location_graph.geojson"
    unique_agents = "id"
    ...
    def __init__(self, pop_size, init_infected, exposure_distance, infection_risk=0.2):
        ...
        #     self.geojson_regions, unique_id=self.unique_regions
        # )
        ...
        LG = gpd.read_file(os.path.join(os.getcwd(), 'location_graph2.geojson'))

        # Generate random location, add agent to grid and scheduler
        for index, row in LG.iterrows():
            # random_location = np.random.randint(low = 0, high = pop_size, size = 1)[0]
